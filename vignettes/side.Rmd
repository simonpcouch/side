...
For complex tasks, you can enable **thinking mode** which shows the model's reasoning process before it responds. Press **Ctrl+T** in the chat input to toggle thinking on or off. When enabled, a collapsible "Thinking" block appears above each response showing how the model approached the problem.

Thinking mode uses native reasoning settings from your provider. For Anthropic and Google Gemini, it sets the thinking budget to 1024, and for OpenAI, it sets the reasoning effort to "medium". Thinking is disabled automatically for models that don't support it and isn't supported for providers other than the big three.
